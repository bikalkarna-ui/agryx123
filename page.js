...
// ─── Anthropic via our /api/ai proxy ─────────────────────────────
// claude-haiku-4-5 = 3-5x faster + cheaper for most tasks
// Use claude-sonnet-4-5 only when deep reasoning needed
const MODEL_FAST  = 'claude-haiku-4-5';
const MODEL_SMART = 'claude-sonnet-4-5';

// ── aiCall: non-streaming, for JSON responses only (syllabus parse) ──
async function aiCall(messages, system, userMeta = {}, useSmart = false) {
  try {
...
async function aiStream(messages, system, onChunk, onDone, userMeta = {}, useSmart = false) {
  try {
    ...
    if (!r.ok) {
      const txt = await r.text();
      const msg = `Error ${r.status}: ${txt.slice(0, 200)}`;
...
function PgPlanner({ profile, deadlines, userMeta={}, onLimitReached }) {
  ...
  async function gen(custom) {
    ...
    setBusy(true); setOut('');
    const dlCtx = deadlines.length ? `\nDeadlines: ${deadlines.slice(0,8).map(x=>`${x.title} (${x.date})`).join(', ')}` : '';
    await aiStream(
      [{role:'user',content:`Study plan for ${profile.name} (${profile.major}, ${profile.year}).${dlCtx}\nSituation: ${d}\n\nCreate a 7-day plan with a table: Day|Time|Task|Duration. Include study strategies and breaks.`}],
      'You are a study coach. Create concise, actionable study plans. Use markdown tables.',
      p=>setOut(p), ()=>setBusy(false),
      userMeta, true  // use Sonnet for complex scheduling
    );
  }
